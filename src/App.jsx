...
const VAPID_KEY="BE6Zayo3RFuXx1dQ_3HQ0HFytM9n7ta1QEF7pHGPpUbGpVqAhkgDYV9M0uLcJYRzLrEvl2yC2d5zM6QuWd8tjEo";
...
async function saveFCMToken(empId,token){
  try{
    // Upsert token
    await fetch(`${API}/vms_fcm_tokens`,{
      method:"POST",
      headers:{...HEADERS,"Prefer":"resolution=merge-duplicates"},
      body:JSON.stringify({employee_id:empId,token,updated_at:new Date().toISOString()})
    });
  }catch(e){console.warn("Save FCM token failed:",e);}
}
...
function RegisterVisitor({onDone}){
  ...
  async function submit(){
    ...
    try{
      ...
      sendPushToHost(payload.department,payload.host_name,payload.name,payload.visitor_id);
